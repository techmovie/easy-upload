...
class UHDExtractor extends GazelleExtractor {
  ...
  extractTitle() {
    ...
    const torrentFileDom = $(
      `#files_${this.torrentId} .filelist_table>tbody>tr:nth-child(2) td`,
    ).eq(0);
    const torrentFileName =
      torrentFilePathDom.text()?.replace(/\//g, '') ||
      torrentFileDom.text()?.replace(/\.(mkv|mp4|avi|mpg|ts|iso)$/i, '');
  ...
  extractMovieNames() {
    ...
    const [movieName = '', movieAkaName = ''] =
      titleText.match(/(.+?)\[/)?.[1].split('/') ?? [];
  ...
  extractImdbUrl() {
    this.info.imdbUrl =
      $('.imovie_title .tooltip.imdb_icon').attr('href') || '';
  }
  ...
  extractMetaInfo() {
    ...
    const size = convertSizeStringToBytes(
      $(`#torrent${this.torrentId} td`).eq(1).text(),
    );
    const infoArray = $(`#torrent${this.torrentId} td:first-child>a`)
      .text()
      .replace(/\s/g, '')
      .split('/');
