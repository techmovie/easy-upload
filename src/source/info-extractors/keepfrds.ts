...
class FRDSExtractor extends NexusPHPExtractor {
  ...
  extractSubtitle () {
    this.info.subtitle = formatTorrentTitle($('#top').text().split(/\s{3,}/)?.[0]?.trim());
  }
  ...
  async extractScreenshots () {
    ...
    await extractImgsFromBBCode(this.info.description.replace(/\[quote\]截图对比[^\n]*\n[^\n]*/gi, ''));
  }
