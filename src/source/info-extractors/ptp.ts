...
class PTPExtractor extends GazelleExtractor {
  ...
  protected extractMediaInfos () {
    ...
    $(`#torrent_${this.torrentId}`).find('.mediainfo.mediainfo--in-release-description').next('blockquote').each(function () {
  ...
  protected extractYear () {
    this.info.year = $('.page__title').text().match(/\[(\d+)\]/)?.[1] ?? '';
  }
  ...
  protected formateDescription (description: string, screenshots: string[], mediaInfoArray: string[]) {
    ...
    screenshots.forEach(screenshot => {
      ...
      // torrents.php?id=78613&torrentid=590102 [img=https://ptpimg.me/yvm3e5.png]
        ...
        if (descriptionData.match(regLegacyFormat)) {
          descriptionData = descriptionData.replace(regLegacyFormat, `[img]${screenshot}[/img]`);
        } else {
    ...
    mediaInfoArray.forEach(mediaInfo => {
      const regStr = new RegExp(`\\[quote\\]\\s*?${replaceRegSymbols(mediaInfo).slice(0, 10000)}`, 'i');
      if (!descriptionData.match(regStr)) {
  ...
  protected getVideoType = (container:string, isRemux:boolean, codes:string, source:string) => {
    let type = '';
    if (isRemux) {
      type = 'remux';
  ...
  protected extractSize () {
    const size = this.torrentHeaderDom.find('.nobr span').attr('title')?.replace(/[^\d]/g, '') ?? '';
